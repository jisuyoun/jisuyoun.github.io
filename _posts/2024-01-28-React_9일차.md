...
layout: single
title:  "React 8일차"
summary: React 공부 8일차-Conditional Rendering
author: YoonJiSu
date: '2024-01-27 21:38:00 +0900'
---      
...
function Toolbar(props) {
    ...
    return (
        <div style={styles.wrapper}>
            {isLoggedIn && <span style={styles.greeting}>환경합니다!</span>}
...
function LandingPage(props) {
    ...
    return (
        <div>
            ...
            />
            <div styles={{padding:16}}>소플과 함께하는 리액트 공부!</div>
        </div>
    );
}
...
이번 실습에서는 로그인 버튼을 누를 경우 isLoggedIn 이 true 가 되면서 ```환영합니다```가 화면에 나타나며, 로그인 버튼이 ```로그아웃```으로 변경되는 코드를 작성하였다.
